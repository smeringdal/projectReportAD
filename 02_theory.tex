...
\label{sec:AD}
Automatic differentiation (AD) is a method that makes the computer derive the derivatives with very little effort from the user. If you have not heard of AD before, the first thing that you might think of algebraic or symbolic differentiation. In this type of differentiation the computer learns the basic rules from calculus like e.g.
\begin{align*}
...
The type of AD I will look further into is the one described by Knut-Andreas Lie in \emph{\citet{lieMrstUrl}}. The method is similar to symbolic differentiation in the way that we implement the differentiation rules, but it differs by instead of differentiating symbols and then inserting values for the symbols, we keep track of the function values and the corresponding values of the derivatives. This is stored in a tuple $[\cdot, \cdot]$. In this way, we can continuously update both the function value and the derivative value for every operation we perform on the function. 
...
It is also necessary to define the chain rule such that for a function h(x),  $h\bigg(\big[f \hspace{0.5em} , \hspace{0.5em} f_x\big]\bigg) = \big[h(f) \hspace{0.5em} , \hspace{0.5em}  f_x\cdot h'(f)\big]$. The only thing that remains to define are the rules concerning elementary functions like
\begin{equation}
    \begin{aligned}
...
But AD is not limited to functions with single variables. Say we have three variables x, y and z. Then the corresponding AD-variables becomes
\begin{equation*}
...
One approach to implementing forward AD
