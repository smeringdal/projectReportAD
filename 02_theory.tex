...
\begin{equation}
    \label{eq:chainRule}
    \frac{\partial f}{\partial x} = \sum_i\left(\frac{\partial f}{\partial u_i}\cdot\frac{\partial u_i}{\partial x}\right).
\end{equation}
Take $f(x) = x\cdot\exp(2x)$ like in the forward AD example \eqref{eq:forwardADExample}. Then we split up the expression into functions $w_1$, $w_2$, $w_3$ and $f$ such that 
\begin{equation*}
w_1 = x \hspace{2em} w_2 = 2w_1 \hspace{2em} w_3 = \exp(w_2)
\end{equation*}
Now the function can be described as $f = w_1\cdot w_3$. If we want the derivative of $f$ with respect to $x$ we can obtain expressions for all $w$'s by using the chain rule \eqref{eq:chainRule}
\begin{align*}
...
\end{equation*}
\textbf{TODO: Fix the notation and finish writing argument. ?See if it is possible to make changes to the forward AD example to make it more similar?}
